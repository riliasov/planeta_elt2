...
### Добавлено
- **DWH Слоеная Архитектура**: Внедрены схемы `raw`, `stg_gsheets`, `lookups` и `analytics` для разделения ответственности.
...
- **Схема Справочников**: Переименована схема `references` -> `lookups` для устранения конфликтов с зарезервированными словами SQL.
- **Права Доступа**: Настроена ролевая модель безопасности. Роль `authenticated` получила права `FULL` (CRUD) для выполнения ETL операций. Роль `anon` лишена доступа к критическим схемам.
